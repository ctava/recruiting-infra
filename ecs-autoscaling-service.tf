...
resource "aws_appautoscaling_policy" "service_down" {
  name               = var.ecs_service_scale_down
  service_namespace  = "ecs"
  resource_id        = "service/${aws_ecs_cluster.main.name}/${aws_ecs_service.service.name}"
  scalable_dimension = "ecs:service:DesiredCount"
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_high" {
  alarm_name          = var.ecs_service_cpu_utilization_high
...
resource "aws_cloudwatch_metric_alarm" "service_cpu_low" {
  alarm_name          = var.ecs_service_cpu_utilization_low
